...
internal class JavadocBlock(private val node: ASTNode) : Block {
  private val children by lazy {
    ...
    val withoutClose = node.text.substringBeforeLast("*/").trim(' ', '\n')
    var start = node.startOffset
    withoutClose.toCharArray().forEachIndexed { index, c ->
      when (c) {
        '\n' -> {
          if (result.isEmpty()) {
            result.add(AbstractLeaf(TextRange(start, node.startOffset + index)))
          } else {
            result.add(AbstractLeaf(
                TextRange(start, node.startOffset + index), myIndent = Indent.getSpaceIndent(1)
            ))
          }
          start = node.startOffset + index + 1
        }
        ' ' -> {
          if (start == node.startOffset + index) start++ // Remove spaces from the start of a line
        }
      }
    }
    if (start < node.startOffset + withoutClose.length) {
      result.add(AbstractLeaf(
          TextRange(start, node.startOffset + withoutClose.length),
          myIndent = Indent.getSpaceIndent(1)
      ))
    }
    // Add the javadoc closer (*/)
    result.add(AbstractLeaf(
        TextRange(node.startOffset + node.textLength - 2, node.startOffset + node.textLength),
        myIndent = Indent.getSpaceIndent(1)
    ))
    result
  }
